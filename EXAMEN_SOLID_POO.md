...
El LSP es uno de los principios SOLID quiere decir que los objetos de una clase que es derivada tienen que poder sustituir a los que estén en su clase sin hacer cambios en la función del programa. Aqui en este ejemplo la clase "Estudiante" hereda la clase "Usuario" y está sobreescribiendo el metodo "calcular_limite_prestamos", entonces si cumple con LSP, porque cualquier función que espere usuario puede recibir un Estudiante sin alterar nada.
_________________________________________________________________
```
...
# Tu código aquí
class Piloto: 
    def vueltas_totales(self):
        return 20

class PilotoInvitado(Piloto):
    def vueltas_totales(self):
        return "Demasiadas vueltas"


# Explicación:
_________________________________________________________________
```
    El problemas aqui es que el método vueltas_totales() en la clase base (Piloto) devuelve siempre un número, mientras que la clase derivada (PilotoInvitado) devuelve un texto.
---
...
```
_________________________________________________________________
Lo viola porque estas obligando que las clases que implementen a depender de métodos que no realmente necesitan, mezclando responsabilidad en un sola interfaz.
_________________________________________________________________
```
...
```
Interface 1: IGestorLibros - Métodos: agregar_libro, buscar_libro

Interface 2: IGestorPrestamos - Métodos: realizar_prestamo

Interface 3: IGeneradorReportesYBackup - Métodos: generar_reporte, hacer_backup
